...
async function loadAdminData() {
  const [profiles, courses, enrollments, teachings, materials, sessions] = await Promise.all([
  ...
  const error = [profiles, courses, enrollments, teachings, materials, sessions].find((result) => result.error)?.error;
  if (error) throw error;
  adminState = { ...adminState, profiles: profiles.data || [], courses: courses.data || [], enrollments: enrollments.data || [], teachings: teachings.data || [], materials: materials.data || [], sessions: sessions.data || [] };
}
...
function renderAdmin(session, name) {
  const views = { usuarios: adminUsersView, cursos: adminCoursesView, asignaciones: adminAssignmentsView, contenido: adminContentView };
  const active = adminState.view;
  app.innerHTML = `<div class="shell admin-shell"><aside><div class="wordmark"><img src="../assets/logos/lumvea-header.png" alt="LUMVEA Educación" /></div><p class="nav-title">ADMINISTRACIÓN</p><nav class="admin-nav" aria-label="Administración del aula"><button id="usuarios-tab" type="button" data-admin-view="usuarios" aria-current="${active === 'usuarios' ? 'page' : 'false'}">Usuarios</button><button id="cursos-tab" type="button" data-admin-view="cursos" aria-current="${active === 'cursos' ? 'page' : 'false'}">Cursos</button><button id="asignaciones-tab" type="button" data-admin-view="asignaciones" aria-current="${active === 'asignaciones' ? 'page' : 'false'}">Asignaciones</button><button id="contenido-tab" type="button" data-admin-view="contenido" aria-current="${active === 'contenido' ? 'page' : 'false'}">Contenido</button></nav><button class="signout" id="signout" type="button">Cerrar sesión</button></aside><main class="content" id="inicio"><header class="topbar"><div><p class="eyebrow">ADMINISTRACIÓN DEL AULA</p><h1>Hola, ${escapeHtml(name)}.</h1><p class="muted">Administra las cuentas, la oferta académica, el contenido y sus asignaciones.</p></div><p class="user">Sesión de administración<br>${escapeHtml(session.user.email)}</p></header><p id="admin-status" class="admin-status" role="status" aria-live="polite"></p>${views[active]()}</main></div>`;
