...
function protectSession() { clearTimeout(timeout); clearTimeout(warningTimeout); clearInterval(countdown); document.querySelector('#session-warning')?.remove(); warningTimeout = setTimeout(async () => { const { data: { session } } = await supabase.auth.getSession(); if (!session) return; let seconds = 30; const warning = document.createElement('div'); warning.id = 'session-warning'; warning.className = 'session-warning'; warning.innerHTML = `<p>Tu sesión cerrará en <strong>${seconds}</strong> segundos por inactividad.</p><button type="button">Extender sesión</button>`; warning.querySelector('button').addEventListener('click', protectSession); document.body.append(warning); countdown = setInterval(() => { seconds -= 1; const count = warning.querySelector('strong'); if (count) count.textContent = String(seconds); }, 1000); }, 150000); timeout = setTimeout(() => supabase.auth.signOut(), 180000); }

function login() {
  clearTimeout(timeout); clearTimeout(warningTimeout); clearInterval(countdown); document.querySelector('#session-warning')?.remove();
  addGlobalNav();
